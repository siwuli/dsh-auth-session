/**
 * dsh-auth-session 核心认证逻辑 (纯函数, 无依赖, 可在任何 Node 环境测试)
 *
...
/** 生成会话令牌 */
function makeToken(secret, user, expiresAtUnixSeconds) {
...
function parseToken(secret, token) {
  ...
  if (payloadB64.length === 0 || sig.length !== expected.length || !crypto.timingSafeEqual(Buffer.from(sig, 'ascii'), Buffer.from(expected, 'ascii'))) {
    return null;
  }
  let data;
  try {
...
/** 从 Cookie 请求头里提取会话令牌 */
function tokenFromCookieHeader(cookieHeader, cookieName) {
...
/** 构造 Set-Cookie 响应头值 */
function sessionCookieHeader(cookieName, token, maxAgeSeconds, secure) {
